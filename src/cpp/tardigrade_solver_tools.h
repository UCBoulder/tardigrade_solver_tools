...
namespace tardigradeSolverTools {
     ...
     */
    typedef errorOut (*NonLinearFunction)(const floatVector &x, const floatMatrix &floatArgs, const intMatrix &intArgs,
     ...
     * \param &residual: The residual vector
     */
    typedef std::function<errorOut(const floatVector &x, const floatMatrix &floatArgs, const intMatrix &intArgs,
                                   floatVector &residual)>
        stdFncNLF;
     ...
     */
    typedef errorOut (*NonLinearFunctionWithJacobian)(const floatVector &x, const floatMatrix &floatArgs,
                                                      const intMatrix &intArgs, floatVector &residual,
                                                      floatMatrix &jacobian, floatMatrix &floatOuts,
                                                      intMatrix &intOuts);
    /**
     * A residual function including the Jacobian
     *
     ...
     * \param &intOuts: Additional integer values to return.
     */
    typedef errorOut (*LagrangianFunctionWithGradient)(const floatVector &x, const floatMatrix &floatArgs,
                                                       const intMatrix &intArgs, floatType &value,
                                                       floatVector &gradient, floatMatrix &floatOuts,
                                                       intMatrix &intOuts);
     ...
     * \param &intOuts: Additional integer values to return.
     */
    typedef std::function<errorOut(const floatVector &, const floatMatrix &, const intMatrix &, floatType &,
                                   floatVector &, floatMatrix &, intMatrix &)>
        stdFncLagrangianG;
    ...
    errorOut newtonRaphson(stdFncNLFJ residual, const floatVector &x0, floatVector &x, bool &convergeFlag,
                           ...
                           const unsigned int maxNLIterations = 20, const floatType tolr = 1e-9,
                           const floatType tola = 1e-9, const floatType alpha = 1e-4,
                           const unsigned int maxLSIterations = 5, const bool resetOuts = false);

    errorOut newtonRaphson(stdFncNLFJ residual, const floatVector &x0, floatVector &x, bool &convergeFlag,
                           bool &fatalErrorFlag, floatMatrix &floatOuts, intMatrix &intOuts,
                           const floatMatrix &floatArgs, const intMatrix &intArgs, solverType &linearSolver,
                           floatMatrix &J, const unsigned int maxNLIterations = 20, const floatType tolr = 1e-9,
                           const floatType tola = 1e-9, const floatType alpha = 1e-4,
                           const unsigned int maxLSIterations = 5, const bool resetOuts = false);
    ...
    errorOut newtonRaphson(stdFncNLFJ residual, const floatVector &x0, floatVector &x, bool &convergeFlag,
                           ...
                           const floatVector &boundValues, const bool boundMode,
                           const unsigned int maxNLIterations = 20, const floatType tolr = 1e-9,
                           const floatType tola = 1e-9, const floatType alpha = 1e-4,
                           const unsigned int maxLSIterations = 5, const bool resetOuts = false);
    ...
    errorOut homotopySolver(stdFncNLFJ residual, const floatVector &x0, floatVector &x, bool &convergeFlag,
                            ...
                            const unsigned int maxNLIterations = 20, const floatType tolr = 1e-9,
                            const floatType tola = 1e-9, const floatType alpha = 1e-4,
                            const unsigned int maxLSIterations = 5, const floatType ds0 = 1.0,
    ...
    errorOut barrierHomotopySolver(stdFncNLFJ residual, const floatType &dt, const floatVector &x0,
                                   ...
                                   const intMatrix &intArgs, const bool &implicitRefine, floatVector &x,
                                   bool &convergeFlag, bool &fatalErrorFlag, floatMatrix &floatOuts, intMatrix &intOuts,
                                   solverType &linearSolver, floatMatrix &J, const unsigned int maxNLIterations = 20,
                                   const floatType tolr = 1e-9, const floatType tola = 1e-9,
                                   const floatType alpha = 1e-4, const unsigned int maxLSIterations = 5,
                                   const bool resetOuts = false);

    errorOut BFGS(stdFncLagrangianG lagrangianGradientFunction, const floatVector &x0, floatVector &x,
                  bool &convergeFlag, bool &fatalErrorFlag, floatMatrix &floatOuts, intMatrix &intOuts,
                  const floatMatrix &floatArgs, const intMatrix &intArgs, const unsigned int maxNLIterations = 20,
                  const floatType tolr = 1e-9, const floatType tola = 1e-9, const floatType alpha = 1e-4,
                  const unsigned int maxLSIterations = 5, const bool resetOuts = false, const floatType stepSize = 1.,
                  const floatType maxdx = -1);
    ...
    errorOut homotopyBFGS(stdFncLagrangianG lagrangianGradientFunction, const floatVector &x0, floatVector &x,
                          ...
                          const unsigned int maxNLIterations = 20, const floatType tolr = 1e-9,
                          const floatType tola = 1e-9, const floatType alpha = 1e-4,
                          const unsigned int maxLSIterations = 5, const floatType ds0 = 1., const floatType dsMin = 0.1,
                          const bool resetOuts = false, const floatType maxdx = -1);
    ...
    errorOut finiteDifference(stdFncNLF fxn, const floatVector &x0, floatMatrix &grad, const floatMatrix &floatArgs,
                              const intMatrix &intArgs, const floatType eps = 1e-6);

    errorOut checkJacobian(stdFncNLFJ residual, const floatVector &x0, const floatMatrix &floatArgs,
                           const intMatrix &intArgs, bool &isGood, const floatType eps = 1e-6,
